...
def main(args):
    ...
    optimizer = AdaLomo(model, lr=args.lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
    model = model.to(torch_dtype).to('cuda:0')
    
    validation_epochs = [1,3,5,7,10,11,12,13,14,15]
...
def train(
    ...
    for global_step in range(args.steps_per_epoch):
        ...
        # loss.backward()
        # optimizer.step()
        optimizer.fused_backward(loss, lr=optimizer.lr if scheduler is None else scheduler.get_last_lr()[0])
